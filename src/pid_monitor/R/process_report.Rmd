...
io_col_spec <- list(
    TIME = col_character(),
    DiskRead = col_number(),
    DiskWrite = col_number(),
    TotalRead = col_number(),
    TotalWrite = col_number()
)
...
```{r io, eval = HAVE_IO}
io_gathered_table <- tidyr::gather(io_file, key = "IO_NAME", value = "IO_SPEED", -TIME)
ggplot(io_gathered_table) +
    geom_line(aes(x = TIME, y = IO_SPEED, color = IO_NAME, group = IO_NAME)) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90)) +
    scale_x_datetime(
        "Time",
    ...
    scale_y_continuous(
        ...
        labs(title = "Mean IO Ultilization plot")
```
